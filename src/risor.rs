...
struct RisorExtension {
    cached_binary_path: Option<String>,
}

#[derive(Clone)]
struct RisorLangserverBinary {
    path: String,
...
impl RisorExtension {
    fn language_server_binary(
    ...
    ) -> Result<RisorLangserverBinary> {
        if let Some(path) = worktree.which("risor-lsp") {
            let environment = worktree.shell_env();
            return Ok(RisorLangserverBinary {
                path,
        ...
        let version_dir = format!("risor-{}", release.version);
        let binary_path = format!("{version_dir}/risor-lsp");
...
impl zed::Extension for RisorExtension {
    fn new() -> Self {
        Self {
...
zed::register_extension!(RisorExtension);
